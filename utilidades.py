...
class db:
    ...
    def vtipo_doc(s):
        if (tipodoc == 'CC' or tipodoc == 'TI' or tipodoc == 'PA'
                or tipodoc == 'CE'):
            return True
        else:
            return False
    ...
    def vcorreo(correo):
        if (len(correo) <= 50 and re.match(
                '^[(a-z0-9\_\-\.)]{3,10}@[(a-z0-9\_\-)]{3,}\.[(a-z)]{2,3}$',
                correo.lower())):
            return True
        else:
            return False
    ...
    def validar_usuario(tipodoc, id, nombre, apellido, fecha, rh, correo,
                        numero):
        if (tipodoc() and id() and nombre() and apellido() and fecha()
                and rh() and correo() and numero()) == f:
            return True
        else:
            return False
    ...
    def visualizar_usuarios(tipodoc, id, nombre, apellido, fecha, rh, correo,
                            numero):
